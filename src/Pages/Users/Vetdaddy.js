...
const VetUsers = () => {
  ...
  const deleteUser = async (id) => {
    const result = await axios.delete(
      `${process.env.REACT_APP_VET}/admin/user/delete/${id}`
    );
  ...
  const sendVip = async (id) => {
    const result = await axios.put(
      `${process.env.REACT_APP_VET}/admin/user/premium`,
      { _id: id, time: vip }
    );

    if (result.data.res === 1) {
      alert(vip + " günlük Premium gönderildi.");
    } else {
      alert("Bir hata oluştu");
    }
  };
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      const result = await axios.get(`${process.env.REACT_APP_VET}/admin/user`);
    ...
    <React.Fragment>
      <div className="page-content">
        <Container fluid>
          <Breadcrumbs
            title="ER2 Studio"
            breadcrumbItem="Kullanıcılar (Vetdaddy)"
          />
          <Row>
            <Col lg={12}>
      ...
      >
        ...
        <Container>
          ...
          <Row className="mb-4 mt-4">
            <div style={{ textAlign: "center" }} className="col-md-6">
              <h4 className="card-title mb-0">PREMIUM : {`${modalData.isPremium}`}</h4>
            </div>
          ...
          <Row style={{ marginTop: 20 }} className="mb-4">
            <div className="col-md-8">
              <select
                ...
                style={{ marginLeft: 20 }}
              >
                <option value={0}>Premium Gün Seçin...</option>
