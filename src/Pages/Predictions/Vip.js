...
const VipPredictions = () => {
  ...
  const handleDelete = (item) => {
    confirmAlert({
      title: "VIP tahmini silmek üzeresin",
      message: "VIP tahmini silmek istediğinize emin misiniz ?",
      buttons: [
        {
          label: "Evet",
  ...
  const deleteMatch = async (item) => {
    const result = await axios.delete(
      `${process.env.REACT_APP_BOSS}/admin/vip/delete/${item._id}`
    );
  ...
  const updateModal = async (item) => {
    const result = await axios.put(
      `${process.env.REACT_APP_BOSS}/admin/vip/update`,
      item
    );
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      const result = await axios.get(`${process.env.REACT_APP_BOSS}/admin/vip`);
      setMatchesData(result.data.bets);
      setLoading(false);
    };
  ...
  return (
    <React.Fragment>
      <div className="page-content">
        <Container fluid>
          <Breadcrumbs title="ER2 Studio" breadcrumbItem="VIP Tahminler" />
          {loading ? (
            <div
              style={{
                display: "flex",
